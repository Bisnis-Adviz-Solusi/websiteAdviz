        ...
        <div className="max-w-6xl mx-auto relative">
          <Card className="shadow-2xl" style={{
            ...
            <CardHeader className="text-primary-foreground">
              <h1 className="text-center text-4xl md:text-5xl font-extrabold mb-3 bg-gradient-to-r from-blue-600 to-green-500 dark:from-blue-400 dark:to-green-400 bg-clip-text text-transparent">
                {t("calculator.title")}
              </h1>
            </CardHeader>
              ...
              <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-4">
                      <div className="space-y-2">
                        <Label htmlFor="gajiPokok">{t("calculator.form.basicSalary")}</Label>
                        <Input
                          id="gajiPokok"
                          type="number"
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="tunjangan">{t("calculator.form.allowance")}</Label>
                        <Input
                          id="tunjangan"
                          type="number"
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="statusPerkawinan">{t("calculator.form.maritalStatus")}</Label>
                        <Select
                          value={formData.statusPerkawinan}
                          onValueChange={(value) => handleInputChange('statusPerkawinan', value)}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder={t("calculator.form.selectStatus")} />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="TK">{t("calculator.form.status.TK")}</SelectItem>
                            <SelectItem value="K">{t("calculator.form.status.K")}</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>

                      <div className="space-y-2">
                        <Label htmlFor="jumlahTanggungan">{t("calculator.form.dependents")}</Label>
                        <Select
                          value={formData.jumlahTanggungan}
                          onValueChange={(value) => handleInputChange('jumlahTanggungan', value)}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder={t("calculator.form.selectDependents")} />
                          </SelectTrigger>
                          <SelectContent>
                            {[0, 1, 2, 3].map(num => (
                ...
                {hasil && (
                  <div className="flex-1" style={{
                    ...
                    <div className="rounded-lg border bg-card shadow-md text-card-foreground">
                      <div className="p-6">
                        <h3 className="text-xl font-bold mb-6">{t("calculator.result.title")}</h3>
                        <div className="grid gap-4">
                          <div className="p-4 rounded-lg bg-muted">
                            <p className="text-sm text-muted-foreground">{t("calculator.result.grossIncome")}</p>
                            <p className="text-lg font-semibold">{formatRupiah(hasil.penghasilanBruto)}</p>
                          </div>

                          <div className="p-4 rounded-lg bg-muted">
                            <p className="text-sm text-muted-foreground">{t("calculator.result.netAnnualIncome")}</p>
                            <p className="text-lg font-semibold">{formatRupiah(hasil.penghasilanNettoTahunan)}</p>
                          </div>

                          <div className="p-4 rounded-lg bg-muted">
                            <p className="text-sm text-muted-foreground">{t("calculator.result.ptkp")}</p>
                            <p className="text-lg font-semibold">{formatRupiah(hasil.ptkp)}</p>
                          </div>

                          <div className="p-4 rounded-lg bg-muted">
                            <p className="text-sm text-muted-foreground">{t("calculator.result.pkp")}</p>
                            <p className="text-lg font-semibold">{formatRupiah(hasil.pkp)}</p>
                          </div>

                          <div className="p-4 rounded-lg bg-primary text-primary-foreground">
                            <p className="text-sm">{t("calculator.result.monthlyPPH")}</p>
